    ...
    <div className="space-y-3">
      {detailList.map((d) => (
        <div key={d.taller.id} className="rounded-xl border border-border p-3.5">
          ...
          <div className="mb-2 flex flex-wrap gap-x-4 gap-y-0.5 text-caption text-muted-foreground">
          ...
          <div className="rounded-md bg-muted/40 px-3 py-2">
            <BulletList items={d.comentario} className="text-caption leading-relaxed" />
          </div>
            ...
            <div className="mt-2.5">
              ...
              <div className="space-y-1.5">
                {d.quejasAsociadas.map((q) => (
                  <div key={q.id} className="rounded-md border border-border/70 bg-white/60 px-2.5 py-1.5 text-caption">
                    ...
                    {!!q.fotos?.length && (
                      <Button size="sm" variant="outline" className="mt-1.5 h-6 px-2 text-micro" onClick={() => setViewPhotos(q.fotos)}>
            ...
            <div className="mt-2.5">
              ...
              <div className="space-y-1.5">
                {[...d.bitacora].sort((a, b) => a.fecha.localeCompare(b.fecha)).map((b) => (
                  <div key={b.id} className="rounded-md border border-border/70 bg-white/60 px-2.5 py-1.5 text-caption">
                    <div className="mb-0.5 flex items-center justify-between gap-2">
                    ...
                    {!!b.fotos?.length && (
                      <Button size="sm" variant="outline" className="mt-1.5 h-6 px-2 text-micro" onClick={() => setViewPhotos(b.fotos)}>
              ...
              <div className="space-y-1.5">
                {(() => {
                  ...
                  return [...grupos.entries()].map(([dia, items]) => (
                    <div key={dia} className="rounded-md border border-border/70 bg-white/60 px-2.5 py-1.5 text-caption">
                      <div className="mb-0.5 font-medium">{fmtDate(dia)}</div>
                      {items.map((c, i) => (
                        <div key={i} className="text-muted-foreground">
                          <span className="text-micro">{fmtHora(c.fecha)}</span> — {c.texto}
                        </div>
                      ))}
                    </div>
                  ));
                ...
                {d.fechasAsociadas.map((fp) => {
                  ...
                  return (
                    <div key={fp.id} className="rounded-md border border-border/70 bg-white/60 px-2.5 py-1.5 text-caption">
                      ...
                      {!!fp.fotos?.length && (
                        <Button size="sm" variant="outline" className="mt-1.5 h-6 px-2 text-micro" onClick={() => setViewPhotos(fp.fotos)}>
