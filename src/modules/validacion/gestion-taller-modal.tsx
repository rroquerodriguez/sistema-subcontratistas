...
export function GestionTallerModal({ taller, validacion, entrega, sub, unidadesProyecto, onSaveValidacion, onSaveEntrega, onClose }: GestionTallerModalProps) {
      ...
      <DialogContent className="max-w-2xl">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as 'liberacion' | 'entrega')}>
          ...
          <TabsContent value="liberacion">
            <ValidacionEditForm validacion={validacion} taller={taller} sub={sub} unidadesProyecto={unidadesProyecto} onSave={onSaveValidacion} onCancel={onClose} />
          </TabsContent>
          <TabsContent value="entrega">
            {liberado ? (
              <EntregaEditForm entrega={entrega} taller={taller} sub={sub} validacion={validacion} onSave={onSaveEntrega} onCancel={onClose} />
            ) : (
