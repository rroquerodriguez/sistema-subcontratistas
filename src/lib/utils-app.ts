...
/** Convierte un objeto Date de JS a YYYY-MM-DD usando sus componentes LOCALES (año/mes/día tal
 * como los muestra el reloj del usuario), nunca .toISOString() — porque toISOString() siempre
 * convierte a UTC, y eso desplaza la fecha un día para cualquier usuario al oeste de Greenwich
 * (incluyendo República Dominicana) durante varias horas de cada tarde/noche. Esta es la causa raíz
 * del bug donde Bitácora (y otras pantallas) mostraban el día siguiente al guardar después de cierta hora. */
...
/** Fecha de HOY en zona horaria local (no UTC). Usar siempre esta función en vez de
 * new Date().toISOString() para "hoy". */
...
export function weekRangeLabel(weekStart: string): string {
  ...
  return `${fmtDate(weekStart)} - ${fmtDate(dateToISOLocal(end))}`;
}
...
/** Fecha ISO (solo día, YYYY-MM-DD) en zona horaria LOCAL a partir de un timestamp ISO datetime completo.
 * No usar .slice(0, 10) sobre el string porque ese string suele venir en UTC. */
...
export function rangoDelMes(mesKey: string): { inicio: string; fin: string } {
  const [y, m] = mesKey.split('-').map(Number);
  const inicio = dateToISOLocal(new Date(y, m - 1, 1));
  const fin = dateToISOLocal(new Date(y, m, 0));
  return { inicio, fin };
}
...
/** Abre un documento HTML (reporte) en una pestaña nueva lista para imprimir/guardar como PDF.
 * Usa un Blob URL en vez de window.open('', '_blank') + document.write, porque los navegadores
 * modernos (Chrome en particular) bloquean ese segundo patrón de forma silenciosa en sitios reales,
 * sin mostrar siquiera el aviso de "pop-up bloqueado". El Blob URL no se considera pop-up genérico. */
export function abrirReporteParaImprimir(html: string) {
  const blob = new Blob([html], { type: 'text/html' });
  const url = URL.createObjectURL(blob);
  const w = window.open(url, '_blank');
  if (!w) {
    alert('El navegador bloqueó la ventana del reporte. Revisa el ícono de "pop-up bloqueado" en la barra de direcciones y permite las ventanas emergentes para este sitio, luego intenta de nuevo.');
