            ...
            <div className="mb-0.5 flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {q.esGeneral ? (
                  ...
                  <span className="text-micro text-muted-foreground">{q.unidades}</span>
                ) : null}
              </div>
            </div>
            {q.descripcion && <div className="text-muted-foreground">{q.descripcion}</div>}
            {!!q.fotos?.length && (
              <Button size="sm" variant="outline" className="mt-1.5 h-6 px-2 text-micro" onClick={() => onViewPhotos(q.fotos)}>
