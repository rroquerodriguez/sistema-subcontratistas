...
export function ResponsabilidadPanel({ resumen }: { resumen: ResumenResponsabilidad }) {
  if (resumen.analizados === 0) {
    return (
      <div className="rounded-lg border border-border bg-muted/20 px-4 py-3 text-caption text-muted-foreground">
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-border p-3.5">
          <div className="mb-1.5 flex items-center gap-1.5 text-caption font-medium">
            <Building2 size={14} className="text-sky-600" />Liberación (nuestra)
          </div>
          <div className="text-display font-semibold leading-none"><Pct valor={resumen.pctLiberacionATiempo} /></div>
          <div className="mt-1 text-caption text-muted-foreground">a tiempo vs. fecha promesa</div>
          {resumen.atrasoLiberacionPromedio != null && resumen.atrasoLiberacionPromedio > 0 && (
            <div className="mt-1.5 text-caption text-destructive">Atraso promedio al liberar: {resumen.atrasoLiberacionPromedio.toFixed(1)} días</div>
          )}
        </div>
        ...
        <div className="rounded-lg border border-border p-3.5">
          ...
          <div className="text-display font-semibold leading-none"><Pct valor={resumen.pctEjecucionATiempo} /></div>
          <div className="mt-1 text-caption text-muted-foreground">dentro del estándar de tiempo</div>
          {resumen.excesoEjecucionPromedio != null && resumen.excesoEjecucionPromedio > 0 && (
            <div className="mt-1.5 text-caption text-destructive">Exceso promedio: {resumen.excesoEjecucionPromedio.toFixed(1)} días laborables</div>
          )}
          {resumen.ejecucionSinEstandar > 0 && (
            <div className="mt-1 text-micro text-muted-foreground">{resumen.ejecucionSinEstandar} entrega(s) sin estándar definido (no evaluadas)</div>
          )}
        </div>
        ...
        <div className="rounded-lg border border-border p-3.5">
          ...
          <div className="text-display font-semibold leading-none"><Pct valor={resumen.pctEntregaATiempo} /></div>
          <div className="mt-1 text-caption text-muted-foreground">entregado en/antes de la promesa</div>
          {resumen.atrasoEntregaPromedio != null && resumen.atrasoEntregaPromedio > 0 && (
            <div className="mt-1.5 text-caption text-destructive">Atraso promedio: {resumen.atrasoEntregaPromedio.toFixed(1)} días</div>
          )}
        </div>
      </div>
      ...
      {(resumen.diasAtrasoNuestro > 0 || resumen.diasAtrasoContratista > 0) && (
        <div className="rounded-lg border border-border bg-muted/20 p-3.5">
          <div className="mb-2 text-caption font-medium">¿De quién fue el atraso? (días acumulados sobre la promesa)</div>
          {(() => {
            ...
            return (
              <>
                <div className="flex h-6 overflow-hidden rounded-md">
                  <div className="flex items-center justify-center bg-sky-500 text-micro font-medium text-white" style={{ width: `${pctN}%` }}>
                    {pctN >= 12 && `${pctN}%`}
                  </div>
                  <div className="flex items-center justify-center bg-amber-500 text-micro font-medium text-white" style={{ width: `${pctC}%` }}>
                    {pctC >= 12 && `${pctC}%`}
                  </div>
                </div>
